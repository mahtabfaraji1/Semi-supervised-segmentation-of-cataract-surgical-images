...
output_dir = '/Users/mahtab/Downloads/SSL4MIS/segmentation_project/model/experiment with labeled=600, unlabeled=10166_cataract1k_v4_600_labeled/output/'
...
for key in range(num_classes):
    if key in metric_dict_a and key in metric_dict_b:
        ...
        if len(metrics_a) > 0 and len(metrics_b) > 0:
            ...
            print(f"t_stat and p_value{class_names[key]}: {t_stat:.2f} : {p_value:.2f}")
            ...
            plt.boxplot([metrics_b, metrics_a], labels=['Supervised', 'Semi-supervised'], patch_artist=True)
...
for key in range(num_classes):
    if key in metric_dict_a and key in metric_dict_b:
        ...
        if len(metrics_a) > 0 or len(metrics_b) > 0:
            ...
            # Add labels for box plots
            labels.append(f"{class_names[key]} (A)")
            labels.append(f"{class_names[key]} (B)")

# Create box plots
plt.figure(figsize=(16, 8))
plt.boxplot(boxplot_data, labels=labels, patch_artist=True, boxprops=dict(facecolor='lightblue', color='blue'))

# Adjust x-ticks and labels
plt.xticks(rotation=45, fontsize=10)
plt.ylabel("Dice Score")
plt.title("Comparison of Dice Scores Across Classes for Method A and Method B")

# Add legend
handles = [plt.Line2D([0], [0], color='lightblue', lw=4, label='Method A'),
           plt.Line2D([0], [0], color='lightcoral', lw=4, label='Method B')]
...
# # Step 6: Visualization
# plt.figure(figsize=(10, 6))
# plt.boxplot([baseline, new_method], labels=['Baseline', 'New Method'], patch_artist=True)
