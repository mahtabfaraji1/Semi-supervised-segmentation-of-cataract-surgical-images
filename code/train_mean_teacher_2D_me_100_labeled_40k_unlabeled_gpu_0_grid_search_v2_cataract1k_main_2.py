...
from utils_1 import losses, ramps
from val_2D_main import test_single_volume,test_single_image,test_single_image_cadis

# Setting variables directly
root_path = '/home/mahtab/segmentation/SSL4MIS/segmentation_env/data/Catarct1k_40k/100_labeled/'
...
def train():
    ...
    base_lr_list = [0.0001,0.001]
    batch_size_list = [16,32,64]
    # ema_decay_list = [0.99]
    ema_decay_list = [0.99]
    # ramp_up_list = 1000
    consistency_list = [0.5]
    # wait_period_list = 2500
    ramp_up_list = [2000]
    wait_period_list = [4000]

    # Generate all combinations of hyperparameters
    hyperparameter_combinations = product(
        base_lr_list, batch_size_list, ema_decay_list, consistency_list, ramp_up_list, wait_period_list
    )

    for base_lr, batch_size, ema_decay, consistency, ramp_up, wait_period in hyperparameter_combinations:
        ...
        exp = f"experiment with labeled={labeled_num}, unlabeled={unlabeled_num}_base_lr={base_lr}_batch_size={batch_size}_ema_decay={ema_decay}_consistency_{consistency}_ramp_up_{ramp_up}_wait_period_{wait_period}_cataract1k_v4_model={model_name}"
        print(exp)
        ...
        for epoch_num in iterator:
            for i_batch, sampled_batch in enumerate(trainloader):
                ...
                if iter_num > 0 and iter_num % 200 == 0:
                    ...
                    if wait >= patience and iter_num>wait_period:
                        print("wait _1:",wait)
            ...
            if wait >= patience or iter_num >= max_iterations:
                print("wait _2:", wait)
                break  # Exit the epoch loop for the current experiment
        # if wait >= patience or iter_num >= max_iterations:
        #     break  # Exit the epoch loop for the current experiment
            # if iter_num >= max_iterations:
                # iter_num = 0
