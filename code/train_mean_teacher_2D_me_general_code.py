...
def train(args):
    ...
    exp = (f"l_num={labeled_num}, unl_num={unlabeled_num}_ema_decay={ema_decay}"
           f"_consistency_{consistency}"
           f"_num_class={num_classes}noise_amount = {noise_amount}_ noise_type={noise_type}"
           )
    print(exp)
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if noise_type == "gaussian_noise":
                noise = torch.clamp(torch.randn_like(
                    unlabeled_volume_batch) * noise_amount, -1* clamping_value, clamping_value)
                ema_inputs = unlabeled_volume_batch + noise
                ema_inputs = torch.clamp(ema_inputs, 0, 255)
                ema_inputs = ema_inputs.squeeze(1)
                ema_inputs = ema_inputs.permute(0, 3, 1, 2)
            elif noise_type == "gaussian_blur":
                ema_inputs = unlabeled_volume_batch.squeeze(1)
                ema_inputs = ema_inputs.permute(0, 3, 1, 2)
                noise_amount = int(noise_amount)
                kernel_size = [noise_amount,noise_amount]
                ema_inputs = F.gaussian_blur(ema_inputs, kernel_size=kernel_size)
            elif noise_type == "impulse_noise":
                ema_inputs = unlabeled_volume_batch.clone()
                prob = noise_amount  # noise_amount should be a float in [0, 1]
                rand = torch.rand_like(ema_inputs)

                # Salt (255)
                ema_inputs[rand < (prob / 2)] = 255
                # Pepper (0)
                ema_inputs[(rand >= (prob / 2)) & (rand < prob)] = 0
            ...
            logging.info(
                'epoch_num %d_ iteration %d : total loss : %f, loss_ce: %f, loss_dice: %f, loss_cons: %f , con_weight: %f' %
                (epoch_num,iter_num, loss.item(), loss_ce.item(), loss_dice.item(), consistency_loss.item(),consistency_weight))
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                logging.info(
                    'iteration %d : mean_dice : %f mean_hd95 : %f wait %d: wait' % (iter_num, performance, mean_hd95, wait))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root_path', type=str,
                        default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/100_labeled_data_semisupervised/',
                        help='Name of Experiment')
    parser.add_argument('--exp', type=str,
                        default='100_labeled_binary_MT', help='experiment_name')
    parser.add_argument('--model_name', type=str,
                        default='unet_resnet', help='model_name')
    parser.add_argument('--num_classes', type=int, default=5,
                        help='output channel of network')
    parser.add_argument('--model_backbone', type=str,
                        default='resnet50', help='model_backbone')
    ...
    parser.add_argument('--ramp_up', type=float,
                        default=1000.0, help='ramp_up')
    parser.add_argument('--ema_decay', type=float, default=0.995, help='ema_decay')
    # parser.add_argument('--apply_noise_unlabeled', action='store_true', dest='apply_noise_unlabeled')
    parser.add_argument('--noise_amount', type=float,
                        default=20, help='noise_amount')
    parser.add_argument('--noise_type', type=str,
                        default='gaussian_noise', help='noise_type')

    parser.add_argument('--device', type=int, default=3, help="CUDA device number (default: 0)")
