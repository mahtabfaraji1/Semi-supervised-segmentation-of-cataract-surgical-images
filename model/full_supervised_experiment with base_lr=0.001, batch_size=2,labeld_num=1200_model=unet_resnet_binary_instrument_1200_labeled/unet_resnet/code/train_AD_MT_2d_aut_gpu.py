...
from utils import losses_ad_mt, ramps
...
def train(args, snapshot_path):
    ...
    def create_model(ema=False):
        ...
        model = net_factory(net_type=args["model"], in_chns=3,
                            class_num=num_classes,device=device)
        model.to(device)
        if ema:
    ...
    # print("Total silices is: {}, labeled slices is: {}".format(total_images, len(labeled_idxs)))
    # logging.info("Total silices is: {}, labeled slices is: {}".format(total_images, labeled_idxs))
    # labeled_idxs = list(range(0, labeled_idxs))
    # unlabeled_idxs = list(range(labeled_idxs, total_images))
    ...
    dice_loss = losses_ad_mt.DiceLoss(num_classes)
    ...
    for epoch_num in iterator:
        ...
        meter_uns_losses = AverageMeter()
        meter_train_losses = AverageMeter()
        ...
        meter_uns_losses_consist = AverageMeter()
        meter_uns_losses_conflict = AverageMeter()
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # 2) getting pseudo labels_four_class
            ...
            with torch.no_grad():
                if not flag_start_self_learning and not args["flag_pseudo_from_student"]:
                    ...
                    img_ulb_w = img_ulb_w.squeeze(1)
                    img_ulb_w = img_ulb_w.permute(0, 3, 1, 2)
            ...
            # 4) forward
            expected_shape = torch.Size([12, 3, 256, 256])


            img_lb_w = img_lb_w.squeeze(1)
            img_lb_w = img_lb_w.permute(0, 3, 1, 2)

            if not img_ulb_s.size() == expected_shape:
                img_ulb_s = img_ulb_s.squeeze(1)
                img_ulb_s = img_ulb_s.permute(0, 3, 1, 2)
            # print(img_lb_w.size())
            # print(img_ulb_s.size())
            ...
            # print(ce_loss(pred_lb, target_lb.long()))
        ...
        if epoch_num % args.get("test_interval_ep", 1) == 0 or iter_num >= max_iterations:
            ...
            for _, sampled_batch in enumerate(valloader):
                ...
                ema_metric_i = test_single_image(
                    sampled_batch["image"], 
                    sampled_batch["label"], 
                    ema_model, 
                    classes=num_classes,device=device)
                ...
                if not flag_start_self_learning:
                    ema_another_metric_i = test_single_image(
                        sampled_batch["image"], 
                        sampled_batch["label"], 
                        ema_model_another, 
                        classes=num_classes,device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cfg', type=str, default='config_2d_aut.yml',
    ...
    parser.add_argument('--res_path', type=str, 
                        default='/home/mahtab/segmentation/SSL4MIS/segmentation_env/results_AD_MT/', help='Path to save resutls')
    parser.add_argument('--exp', type=str,
                        default='AD-MT_all_data', help='experiment_name')
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="confidence threshold for using pseudo-labels_four_class",
    )
    ...
    cfgs_file = args['cfg']
    cfgs_file = os.path.join('/home/mahtab/segmentation/SSL4MIS/segmentation_env/cfg',cfgs_file)
