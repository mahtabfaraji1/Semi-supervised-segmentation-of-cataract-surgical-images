...
def net_factory(class_num,net_type="unet", in_chns=3,device = torch.device("cuda" if torch.cuda.is_available() else "cpu")):
    if net_type == "unet":
        ...
        net = UNetResnet(class_num, in_channels=in_chns, backbone='resnet50', pretrained=True, freeze_bn=False,
                 freeze_backbone=False).to(device)
    elif net_type == "deeplabv3_resnet":
        net = DeepLab(class_num, in_channels=in_chns, backbone='resnet', pretrained=True, freeze_bn=False,
                 freeze_backbone=False).to(device)
    elif net_type == "enet":
