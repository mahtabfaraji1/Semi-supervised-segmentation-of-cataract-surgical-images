...
#                     default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/100_labeled_data_semisupervised/test/labels_four_class',
#                     help='experiment_name')
# parser.add_argument('--exp', type=str,
#                     default='/Volumes/ADATA HD720/PhD_UIC/SSL4MIS/segmentation_project/model_1/experiment with labeled=100, unlabeled=40042_ema_decay=0.99_consistency_0.5_ramp_up_800_wait_period_2000_cataract1k_v4_model=deeplabv3_resnet_100_labeled/',
#                     help='experiment_name')
#
# # parser.add_argument('--exp', type=str,
# #                     default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/model/experiment with labeled=100, unlabeled=40042_cataract1k_v4_model=deeplabv3_resnet_100_labeled/',
# #                     help='experiment_name')
# parser.add_argument('--model', type=str, default='deeplabv3_resnet', help='model_name')
# # parser.add_argument('--model', type=str, default='ViT_Seg', help='model_name')6
# parser.add_argument('--num_classes', type=int, default=5, help='output channel of network')
# parser.add_argument('--output_dir', type=str, default='/Volumes/ADATA HD720/PhD_UIC/SSL4MIS/segmentation_project/model_1/experiment with labeled=100, unlabeled=40042_ema_decay=0.99_consistency_0.5_ramp_up_800_wait_period_2000_cataract1k_v4_model=deeplabv3_resnet_100_labeled/output',
...
#     save_mode_path = os.path.join(snapshot_path, '{}_best_ema_model.pth'.format(FLAGS.model)) # mean teacher and supervised UNEt
...
def test_single_image(image_path, label_path, net, device, patch_size):
    image = cv2.imread(image_path)
    label = cv2.imread(label_path, cv2.IMREAD_GRAYSCALE)
    x, y = image.shape[0], image.shape[1]
    resized_image = zoom(image, (patch_size / x, patch_size / y, 1), order=0)
    resized_label = zoom(label, (patch_size / x, patch_size / y), order=0)
    input_tensor = torch.from_numpy(resized_image).permute(2, 0, 1).unsqueeze(0).float().to(device)
    net.eval()
    with torch.no_grad():
        out = net(input_tensor)
        prediction = torch.argmax(torch.softmax(out, dim=1), dim=1).squeeze(0).cpu().numpy()
    prediction = zoom(prediction, (x / patch_size, y / patch_size), order=0)
    metrics_per_class = {}
    for cls in np.unique(label):
        ...
        pred_mask = (prediction == cls).astype(np.uint8)
        label_mask = (label == cls).astype(np.uint8)
        metrics_per_class[cls] = calculate_metric_percase(pred_mask, label_mask) if np.any(pred_mask) else (0, 0, 0, 0)
    return metrics_per_class

def find_model_files(model_root_dir):
    model_files = []
    for root, dirs, files in os.walk(model_root_dir):
        dirs[:] = [d for d in dirs if not d.startswith("._")]
        for folder in dirs:
            file = os.path.join(folder,"deeplabv3_resnet","deeplabv3_resnet_best_ema_model.pth")
            # if file == "deeplabv3_resnet_best_ema_model.pth":
            model_files.append(os.path.join(root, file))
    return model_files

def evaluate_model(model_path, image_dir, label_dir, num_classes, patch_size, device):
    net = net_factory(net_type="deeplabv3_resnet", in_chns=3, class_num=num_classes)
    net.load_state_dict(torch.load(model_path, map_location=device), strict=False)
    net.to(device)
    metric_dict = {key: [] for key in range(1, num_classes)}
    ...
    for image_file in tqdm(image_files):
        ...
        if not os.path.exists(label_path):
            ...
            continue
        metrics = test_single_image(image_path, label_path, net, device, patch_size[0])
...
def main(args):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_files = find_model_files(args.model_root_dir)
    results_path = os.path.join(args.output_dir, "model_performance.txt")
    with open(results_path, "w") as results_file:
        for model_file in model_files:
            print(f"Evaluating model: {model_file}")
            results_file.write(f"Model: {model_file}\n")
            metrics = evaluate_model(model_file, args.image_dir, args.label_dir, args.num_classes, args.patch_size, device)
            for key, values in metrics.items():
                if values:
                    dice_scores, jc_scores, asd_scores, hd95_scores = zip(*values)
                    results_file.write(f"Class {key}: Dice: {np.mean(dice_scores):.2f} +/- {np.std(dice_scores):.2f}, "
                                       f"HD95: {np.mean(hd95_scores):.2f} +/- {np.std(hd95_scores):.2f}\n")
            results_file.write("\n")
    print(f"Results saved to {results_path}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--image_dir', type=str, default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/100_labeled_data_semisupervised/test/images')
    parser.add_argument('--label_dir', type=str, default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/100_labeled_data_semisupervised/test/labels_four_class')
    parser.add_argument('--model_root_dir', type=str, default= '/Volumes/ADATA HD720/PhD_UIC/SSL4MIS/segmentation_project/model_5/')
    parser.add_argument('--num_classes', type=int, default=5, help='Number of output classes')
    parser.add_argument('--patch_size', type=list, default=[256, 256], help='Patch size of network input')
    parser.add_argument('--output_dir', type=str, default= '/Volumes/ADATA HD720/PhD_UIC/SSL4MIS/segmentation_project/model_5/')
