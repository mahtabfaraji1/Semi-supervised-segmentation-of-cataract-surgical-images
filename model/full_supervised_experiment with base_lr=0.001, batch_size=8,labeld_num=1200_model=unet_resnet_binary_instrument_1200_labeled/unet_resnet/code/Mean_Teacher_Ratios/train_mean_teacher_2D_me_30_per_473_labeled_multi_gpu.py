...
exp = 'Experiment_labeled_30_per_473_labeled_e30000_consistency_1_v4'
model_name = 'unet'
...
def train():
    ...
    labeled_images_file = open(os.path.join(snapshot_path, 'labeled_images_30_per.txt'), 'w')
    unlabeled_images_file = open(os.path.join(snapshot_path, 'unlabeled_images_30_per.txt'), 'w')
    ...
    labeled_image_names = db_train.get_image_names(labeled_idxs)
    ...
    def create_model(ema=False):
        ...
        model = torch.nn.DataParallel(model)  # Add this for multi-GPU support
...
if __name__ == "__main__":
    ...
    snapshot_path = "../model/{}_{}_labeled/{}".format(exp, labeled_num, model_name)
    if not os.path.exists(snapshot_path):
        os.makedirs(snapshot_path)
    if os.path.exists(snapshot_path + '/code'):
        shutil.rmtree(snapshot_path + '/code')
    shutil.copytree('Mean_Teacher_Ratios', snapshot_path + '/code', shutil.ignore_patterns(['.git', '__pycache__']))
