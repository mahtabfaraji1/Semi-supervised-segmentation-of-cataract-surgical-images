...
def train():
    ...
    for ramp_up in ramp_up_list:
        ...
        exp = f"experiment with ema_decay={ema_decay}, ramp_up={ramp_up},consistency_weight={consistency},wait_period={wait_period},labeled={labeled_num}, unlabeled={unlabeled_num}_cadis_focal"
        print(exp)
        ...
        for epoch_num in iterator:
            for i_batch, sampled_batch in enumerate(trainloader):
                ...
                # loss_ce = focal_loss(outputs[:labeled_bs], label_batch[:][:labeled_bs].long())
                ...
                supervised_loss = 0.5 * (loss_dice + loss_focal)
                ...
                logging.info(
                    'iteration %d : total loss : %f, loss_focal: %f, loss_dice: %f, loss_cons: %f , con_weight: %f' %
                    (iter_num, loss.item(), loss_focal.item(), loss_dice.item(), consistency_loss.item(),consistency_weight))
