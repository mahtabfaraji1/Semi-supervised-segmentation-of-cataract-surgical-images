...
from val_2D import test_single_image_cadis
import random
...
def apply_augmentations(image_batch):
    augmented_images = []  # List to store augmented images
    for idx, image in enumerate(image_batch):
        # Random cropping
        cropper = v1.RandomAffine(degrees=0, translate=(0.2, 0.2), scale=(0.8, 1.2), shear=10)
        augmented_image = cropper(image)

        jitter = v1.ColorJitter(brightness=(0.5,1.5), contrast=(1), saturation=(0.5,1.5), hue=(-0.1,0.1))
        augmented_image_2 = jitter(augmented_image)


        guassian_blur = v1.GaussianBlur(3, sigma=(0.1, 2.0))
        augmented_image_3 = guassian_blur(augmented_image_2)

        augmented_images.append(augmented_image_3)
...
def train():
    ...
    for ramp_up in ramp_up_list:
        ...
        consistency = 1.5 #0.1
        ramp_up = ramp_up
        wait_period = 1000 # 2500
        labeled_bs = int((batch_size / 4) / 2)
        # define experiment name
        exp = f"experiment with ema_decay={ema_decay}, ramp_up={ramp_up},consistency_weight={consistency},wait_period={wait_period},labeled={labeled_num}, unlabeled={unlabeled_num}_cadis_v5"
        print(exp)
        ...
        for epoch_num in iterator:
            for i_batch, sampled_batch in enumerate(trainloader):
                ...
                unlabeled_volume_batch_tr = volume_batch[labeled_bs:]
                labeled_volume_batch = volume_batch[:labeled_bs]
                unlabeled_volume_batch_st = volume_batch[labeled_bs:]

                noise = torch.clamp(torch.randn_like(
                    unlabeled_volume_batch_tr) * 0.1, -0.2, 0.2)
                # noise = torch.clamp(torch.randn_like(
                #     unlabeled_volume_batch) * 0., -40, 40)
                ema_inputs = unlabeled_volume_batch_tr + noise
                ema_inputs = torch.clamp(ema_inputs, 0, 1)
                ema_inputs = ema_inputs.squeeze(1)
                unlabeled_volume_batch_with_noise = ema_inputs.permute(0, 3, 1, 2)
                ema_inputs = apply_augmentations(unlabeled_volume_batch_with_noise)

                labeled_volume_batch = labeled_volume_batch.squeeze(1)
                labeled_volume_batch = labeled_volume_batch.permute(0, 3, 1, 2)

                unlabeled_volume_batch_st = unlabeled_volume_batch_st.squeeze(1)
                unlabeled_volume_batch_st = unlabeled_volume_batch_st.permute(0, 3, 1, 2)

                outputs = model(labeled_volume_batch)
                outputs_soft = torch.softmax(outputs, dim=1)

                outputs_unlabeled_st = model(unlabeled_volume_batch_st)
                outputs_unlabeled_st_soft = torch.softmax(outputs_unlabeled_st, dim=1)

                with torch.no_grad():
                    ema_output = ema_model(ema_inputs)
                    ema_output_soft = torch.softmax(ema_output, dim=1)
                # loss_ce = ce_loss(outputs[:labeled_bs], label_batch[:][:labeled_bs].long())
                loss_ce = ce_loss(outputs, label_batch[:][:labeled_bs].long())
                # loss_dice = dice_loss(
                #     outputs_soft[:labeled_bs], label_batch[:labeled_bs].unsqueeze(1))
                loss_dice = dice_loss(
                    outputs_soft, label_batch[:labeled_bs].unsqueeze(1))
                supervised_loss = 0.5 * (loss_dice + loss_ce)

                consistency_loss = torch.mean((outputs_unlabeled_st_soft - ema_output_soft) ** 2)
