...
root_path = '/home/mahtab/segmentation/SSL4MIS/segmentation_env/data/cataract1k/1k_experiments/'
# root_path = '/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/cataract1k/'
exp = 'Experiment_labeled_100_labeled_1k_unlabeled_gpu_0_resnet'
...
def train():
    global model_name, base_lr, num_classes, batch_size, max_iterations,num_classes
    ...
    for epoch_num in iterator:
        ...
        consistency_weight = get_current_consistency_weight(rampup_length=ramp_up, current=epoch_num,
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # w_t = wt(rampup_length=ramp_up, current=epoch, alpha=consistency,
            #          wait_period=wait_period)  # Get unsupervised weight for the epoch
            # w_t = wt(rampup_length=ramp_up, current=epoch_num, alpha=consistency,
            #          wait_period=wait_period)

            # consistency_weight = get_current_consistency_weight(iter_num//150)
            consistency_loss = torch.mean((outputs_soft[labeled_bs:] - ema_output_soft) ** 2)
            # if epoch_num < wait_period:
        ...
        if epoch_num > 0 and epoch_num % 10 == 0:
            ...
            logging.info(
                'iteration %d : mean_dice : %f mean_hd95 : %f' % (epoch_num, performance, mean_hd95))
            model.train()
