...
def train():
    ...
    for ramp_up in ramp_up_list:
        ...
        exp = f"experiment with ema_decay={ema_decay}, ramp_up={ramp_up},consistency_weight={consistency},wait_period={wait_period},labeled={labeled_num}, unlabeled={unlabeled_num}_cadis_w_CE"
        print(exp)
        ...
        masks_path = os.path.join(root_path, 'train', 'labels')
        ce_weights = losses.calculate_weights_ce(masks_path)
        ce_weights = torch.tensor(ce_weights, dtype=torch.float)

        # compute_class_weight(class_weight="balanced", classes=np.unique(y), y=y)
        ce_loss = CrossEntropyLoss(weight=ce_weights)
        ...
        for epoch_num in iterator:
            for i_batch, sampled_batch in enumerate(trainloader):
                ...
                logging.info(
                    'iteration %d : total loss : %f, loss_w_ce: %f, loss_dice: %f, loss_cons: %f , con_weight: %f' %
                    (iter_num, loss.item(), ce_loss.item(), loss_dice.item(), consistency_loss.item(),consistency_weight))
