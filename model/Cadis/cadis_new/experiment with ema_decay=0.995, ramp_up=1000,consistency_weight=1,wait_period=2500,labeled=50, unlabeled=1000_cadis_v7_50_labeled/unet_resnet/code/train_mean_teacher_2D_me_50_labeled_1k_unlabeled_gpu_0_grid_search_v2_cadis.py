...
from dataloaders.dataset_V3 import BaseDataSets, RandomGenerator, TwoStreamBatchSampler,StrongAugment,ValidationDataGenerator
...
def apply_augmentations(image_batch):
    ...
    for idx, image in enumerate(image_batch):
        ...
        augmented_images.append(augmented_image_2)
...
def train():
    ...
    for ramp_up in ramp_up_list:
        ...
        exp = f"experiment with ema_decay={ema_decay}, ramp_up={ramp_up},consistency_weight={consistency},wait_period={wait_period},labeled={labeled_num}, unlabeled={unlabeled_num}_cadis_v7"
        print(exp)
        ...
        db_train = BaseDataSets(base_dir=root_path, split="train", num=None,
                                transform= transforms.Compose([ RandomGenerator(patch_size)]),
                                ops_strong = transforms.Compose([StrongAugment(patch_size)])
                                )


        db_val = BaseDataSets(base_dir=root_path, split="val",transform= transforms.Compose([ ValidationDataGenerator(patch_size)]))
        ...
        for epoch_num in iterator:
            for i_batch, sampled_batch in enumerate(trainloader):
                ...
                input_batch,ema_input_batch, label_batch = sampled_batch['image'], sampled_batch['image_ema'],sampled_batch['label']
                input_batch, ema_input_batch, label_batch = input_batch.to(device),ema_input_batch.to(device), label_batch.to(device)
                ...
                # unlabeled_volume_batch = ema_input_batch[labeled_bs:]
                ema_input_batch = ema_input_batch.squeeze(1)
                ema_input_batch = ema_input_batch.permute(0, 3, 1, 2)
                ...
                with torch.no_grad():
                    ema_output = ema_model(ema_input_batch)
                ...
                consistency_loss = torch.mean((outputs_soft[labeled_bs:] - ema_output_soft[labeled_bs:]) ** 2)
                ...
                logging.info(
                    'iteration %d : total loss : %f, loss_ce: %f, loss_dice: %f, loss_cons: %f , consistency_weight:%f' %
                ...
                if iter_num > 0 and iter_num % 200 == 0:
                    ...
                    for i_batch, sampled_batch in enumerate(valloader):
                        metric_i = test_single_image_cadis(
                            sampled_batch["image"], sampled_batch["label"], ema_model, classes=num_classes,device=device)
