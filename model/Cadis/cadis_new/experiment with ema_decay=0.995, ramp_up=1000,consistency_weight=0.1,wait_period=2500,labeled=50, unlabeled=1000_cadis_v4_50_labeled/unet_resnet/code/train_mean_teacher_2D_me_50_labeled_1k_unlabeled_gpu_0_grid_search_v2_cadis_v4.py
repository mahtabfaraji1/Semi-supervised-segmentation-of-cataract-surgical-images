...
def train():
    ...
    for ramp_up in ramp_up_list:
        ...
        exp = f"experiment with ema_decay={ema_decay}, ramp_up={ramp_up},consistency_weight={consistency},wait_period={wait_period},labeled={labeled_num}, unlabeled={unlabeled_num}_cadis_v4"
        print(exp)
        ...
        for epoch_num in iterator:
            for i_batch, sampled_batch in enumerate(trainloader):
                ...
                noise = torch.clamp(torch.randn_like(
                    unlabeled_volume_batch) * 0.3, -0.2, 0.2)
                # noise = torch.normal(mean=0, std=25.0, size=unlabeled_volume_batch.shape).to(device)
                noisy_tensor = unlabeled_volume_batch + noise
                # noisy_tensor = torch.clamp(noisy_tensor, 0.0, 255.0).to(torch.uint8)
                ema_inputs = noisy_tensor.squeeze(1).to(torch.uint8)
