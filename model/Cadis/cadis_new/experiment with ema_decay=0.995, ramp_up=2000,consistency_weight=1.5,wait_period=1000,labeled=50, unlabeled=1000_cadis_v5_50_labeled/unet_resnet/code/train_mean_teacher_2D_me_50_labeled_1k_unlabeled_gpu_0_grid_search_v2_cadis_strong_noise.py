...
from dataloaders.dataset_V3 import BaseDataSets, RandomGenerator, TwoStreamBatchSampler
from networks.net_factory import net_factory
from utils import losses, ramps
from val_2D import test_single_volume,test_single_image,test_single_image_cadis

# Setting variables directly
root_path = '/home/mahtab/segmentation/SSL4MIS/segmentation_env/data/CaDis/50_labeled_data_semisupervised/'
...
def find_label_unlabel_index(db_train):
    ...
    # Find indices of images that do not have corresponding labels
...
def train():
    ...
    for ramp_up in ramp_up_list:
        ...
        exp = f"experiment with ema_decay={ema_decay}, ramp_up={ramp_up},consistency_weight={consistency},wait_period={wait_period},labeled={labeled_num}, unlabeled={unlabeled_num}_cadis_ema_bs_half_modifies_noise_3"
        print(exp)
        ...
        noise_transform = transforms.Compose([
            ...
            transforms.RandomAffine(degrees=0, translate=(0.2, 0.2), scale=(0.8, 1.2), shear=10),  # Random affine
        ...
        for epoch_num in iterator:
            for i_batch, sampled_batch in enumerate(trainloader):
                ...
                volume_batch, label_batch = volume_batch.to(device), label_batch.to(
                    device)  # Move data to the correct device

                # unlabeled_data
                unlabeled_data_batch = volume_batch[labeled_bs:]

                # labeled_data
                st_model_labeled_data_input = volume_batch[:labeled_bs]
                st_model_labeled_data_gt = label_batch[:labeled_bs]
                st_model_labeled_data_input = st_model_labeled_data_input.float()
                st_model_labeled_data_input = st_model_labeled_data_input / 255.0
                st_model_labeled_data_input = st_model_labeled_data_input.squeeze(1)
                st_model_labeled_data_input = st_model_labeled_data_input.permute(0, 3, 1, 2)

                # Add noise to unlabeled data with two different noise
                noise_1 = torch.clamp(torch.randn_like(unlabeled_data_batch.float()) * 0.05, -0.2, 0.2)
                unlabeled_data_batch_1 = unlabeled_data_batch.float()
                unlabeled_data_batch_1 = unlabeled_data_batch_1 / 255.0
                # tr_model_unlabeled_data_input = torch.clamp(unlabeled_data_batch_1 + noise_1, 0, 1)
                tr_model_unlabeled_data_input = unlabeled_data_batch_1.squeeze(1)
                tr_model_unlabeled_data_input = tr_model_unlabeled_data_input.permute(0, 3, 1, 2)
                tr_model_unlabeled_data_input = noise_transform(tr_model_unlabeled_data_input)

                # noise_2 = torch.clamp(torch.randn_like(unlabeled_data_batch.float()) * 0.1, -0.2, 0.2)
                unlabeled_data_batch_2 = unlabeled_data_batch.float()
                unlabeled_data_batch_2 = unlabeled_data_batch_2 / 255.0
                # st_model_unlabeled_data_input = torch.clamp(unlabeled_data_batch_2 + noise_2, 0, 1)
                st_model_unlabeled_data_input = unlabeled_data_batch_2.squeeze(1)
                st_model_unlabeled_data_input = st_model_unlabeled_data_input.permute(0, 3, 1, 2)

                # feed data to student model
                st_model_output_labeled_data = model(st_model_labeled_data_input)
                st_model_output_labeled_data_soft = torch.softmax(st_model_output_labeled_data, dim=1)

                # feed unlabeled data to student model
                # st_model_output_unlabeled_data = model(st_model_unlabeled_data_input.float())
                st_model_output_unlabeled_data = model(st_model_unlabeled_data_input)
                st_model_output_unlabeled_data_soft = torch.softmax(st_model_output_unlabeled_data, dim=1)

                # feed unlabeled data to teacher model
                with torch.no_grad():
                    tr_model_output_unlabeled_data = ema_model(tr_model_unlabeled_data_input)
                    tr_model_output_unlabeled_data_soft = torch.softmax(tr_model_output_unlabeled_data, dim=1)

                # calculate supervised loss
                loss_ce = ce_loss(st_model_output_labeled_data, st_model_labeled_data_gt.long())
                loss_dice = dice_loss(
                    st_model_output_labeled_data_soft, st_model_labeled_data_gt.unsqueeze(1))

                supervised_loss = 0.5 * (loss_dice + loss_ce)
                # calculate consistency supervised loss
                consistency_loss = torch.mean((st_model_output_unlabeled_data_soft - tr_model_output_unlabeled_data_soft) ** 2)
                ...
                logging.info(
                    ...
                    (iter_num, loss.item(), loss_ce.item(), loss_dice.item(), consistency_loss.item()))

                if iter_num > 0 and iter_num % 200 == 0:
                    model.eval()
                    metric_list = 0.0
                    for i_batch, sampled_batch in enumerate(valloader):
                        metric_i = test_single_image_cadis(
