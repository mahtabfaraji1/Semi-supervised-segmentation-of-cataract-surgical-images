...
def train():
    ...
    for ramp_up in ramp_up_list:
        ...
        exp = f"experiment with ema_decay={ema_decay}, ramp_up={ramp_up},consistency_weight={consistency},wait_period={wait_period},labeled={labeled_num}, unlabeled={unlabeled_num}_cadis_ema_bs_half"
        print(exp)
        ...
        for epoch_num in iterator:
            for i_batch, sampled_batch in enumerate(trainloader):
                ...
                noise_1 = torch.clamp(torch.randn_like(unlabeled_data_batch.float()) * 0.1, -0.2, 0.2)
                ...
                st_model_unlabeled_data_input = st_model_unlabeled_data_input.squeeze(1)
                st_model_unlabeled_data_input = st_model_unlabeled_data_input.permute(0, 3, 1, 2)

                # feed data to student model
                st_model_output_labeled_data = model(st_model_labeled_data_input.float())
                ...
                with torch.no_grad():
                    tr_model_output_unlabeled_data = ema_model(tr_model_unlabeled_data_input.float())
