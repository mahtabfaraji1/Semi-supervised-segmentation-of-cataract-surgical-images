...
parser.add_argument('--image_dir', type=str,
                    default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/CaDISv2/test/Video02/Images',
                    help='Name of Experiment')
parser.add_argument('--label_dir', type=str,
                    default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/CaDISv2/test/Video02/Labels',
                    help='experiment_name')
parser.add_argument('--exp', type=str,
                    default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/model/semi_supervised_1k_HP/experiment with ema_decay=0.995, ramp_up=1000,consistency_weight=0.1,wait_period=2500_1k_900_labeld_50_labeled/',
                    help='experiment_name')
...
parser.add_argument('--output_dir', type=str, default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/model/semi_supervised_1k_HP/experiment with ema_decay=0.995, ramp_up=1000,consistency_weight=0.1,wait_period=2500_1k_900_labeld_50_labeled/output/',
...
def test_single_image(image_path, label_path, net, output_dir, FLAGS):
    ...
    # Add each instrument label to the mapping with the value 4 (Instruments)
    # label_mapping.update({label: 4 for label in instruments_labels})
    ...
    # Apply label mapping to resized label
    # mapped_label = np.vectorize(lambda x: label_mapping.get(x, 0))(resized_label)
    ...
    net.eval()
    mapped_label = np.vectorize(lambda x: label_mapping.get(x, 0))(label)
    # Run inference on the network
    with torch.no_grad():
        out_main = net(input_tensor)
        out = torch.argmax(torch.softmax(out_main, dim=1), dim=1).squeeze(0)
        prediction = zoom(out.cpu().detach().numpy(), (x / patch_size, y / patch_size), order=0)
    mapped_prediction = np.vectorize(lambda x: label_mapping.get(x, 0))(prediction)

    # Save the visualized results
    file_name = os.path.basename(image_path).split('.')[0]
    visualize_and_save(image, mapped_label, prediction, output_dir, file_name)

    # Find the unique classes in the label
    unique_classes = np.unique(label)

    # Calculate metrics for the detected classes
    # unique_classes = np.unique(mapped_label)
    metrics_per_class = {}
    for cls in unique_classes:
        if cls == 0:  # Skip background
            continue
        pred_mask = (mapped_prediction == cls).astype(np.uint8)
        label_mask = (label == cls).astype(np.uint8)

        # Ensure pred_mask and label_mask are resized back to original shape
        # pred_mask = zoom(pred_mask, (x / patch_size, y / patch_size), order=0)
        # label_mask = zoom(label_mask, (x / patch_size, y / patch_size), order=0)
