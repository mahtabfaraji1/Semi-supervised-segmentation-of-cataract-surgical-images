...
parser.add_argument('--exp', type=str,
                    default='Experiment_cnn_transformer_labeled_all', help='experiment_name')
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_image(
                        sampled_batch["image"], sampled_batch["label"], model1, classes=num_classes, device =device, patch_size= args.patch_size)
                ...
                for i_batch, sampled_batch in enumerate(valloader):
                    metric_i = test_single_image(
                        sampled_batch["image"], sampled_batch["label"], model2, classes=num_classes, device=device,patch_size= args.patch_size)
                    metric_list += np.array(metric_i)
                metric_list = metric_list / len(db_val)
                for class_i in range(num_classes-1):
