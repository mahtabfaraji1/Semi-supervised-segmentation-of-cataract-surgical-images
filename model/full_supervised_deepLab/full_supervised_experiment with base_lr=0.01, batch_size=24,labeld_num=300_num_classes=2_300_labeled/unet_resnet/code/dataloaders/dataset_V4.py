...
class BaseDataSetsIncludesUnlabeled(Dataset):
    def __init__(self, base_dir=None, split="train", num=None, transform=None, unlabeled=False):
        ...
        if not self.unlabeled:
            ...
            self.label_paths = sorted(glob(os.path.join(self._base_dir, split, 'labels_four_class', '*.png')))
            assert len(self.image_paths) == len(self.label_paths), "Mismatch between images and labels_four_class"
        else:
            self.image_paths = sorted(glob(os.path.join(self._base_dir, split,  'images','unlabeled', '*.png')))
            self.label_paths = [None] * len(self.image_paths)  # No labels_four_class for unlabeled data
...
class BaseDataSets(Dataset):
    def __init__(
        ...
        # Ensure the number of images matches the number of labels_four_class
        # assert len(self.image_paths) == len(self.label_paths), "Mismatch between images and labels_four_class"
    ...
    def __getitem__(self, idx):
        ...
        label_path = image_path.replace('images', 'labels_four_class')
        ...
        if label_path in self.label_paths:
            ...
            # Convert images and labels_four_class to numpy arrays

            label = np.array(label)
        else:
            label = np.zeros((image.shape[0], image.shape[1]))
            label = np.array(label)
        sample = {"image": image, "label": label, "unlabeled_idxs": self.unlabeled_idxs,"idx":idx}
...
def random_noise(image, mean, sigma):
    """
    Randomly rotates and flips an image, and also applies the same transformation to the label if it exists.
    """
    ...
    # Generate random noise (normal distribution, mean=0, std_dev=25)
    ...
    return noisy_image.astype(np.uint8)  # Always return both image and label (label may be None)
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        image, label, unlabeled_idxs,idx= sample["image"], sample.get("label", None), sample["unlabeled_idxs"], sample["idx"] # Ensure label can be None
        ...
        if unlabeled_idxs is not None and idx in unlabeled_idxs:
            mean, sigma = 0, 20
            image = random_noise(image,mean, sigma)
