...
parser.add_argument('--image_dir', type=str,
                    default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/CVS_data/IODA/test/images',
                    help='Name of Experiment')
parser.add_argument('--label_dir', type=str,
                    default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/CVS_data/IODA/test/labels',
                    help='experiment_name')
parser.add_argument('--exp', type=str,
                    default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/model/full_supervised_experiment with base_lr=0.01, batch_size=32,labeld_num=432_model=unet_resnet_FIVES_HRF_432_labeled/',
                    help='experiment_name')
...
parser.add_argument('--output_dir', type=str, default='/Users/mahtab/Downloads/SSL4MIS/segmentation_project/model/full_supervised_experiment with base_lr=0.01, batch_size=32,labeld_num=432_model=unet_resnet_FIVES_HRF_432_labeled/output_ioda/',
...
#     if metrics_per_class:
#         for cls_id, (dice, hd95) in metrics_per_class.items():
...
def visualize_and_save(image, prediction, output_path, file_name, num_classes=5):
    """
    ...
    if raw_image.max() <= 1:  # Assuming the image is normalized between 0 and 1
        image = (raw_image * 255).astype(np.uint8)
    ...
    # Concatenate the images horizontally
    total_width = image_pil.width + prediction_pil.width
    max_height = max(image_pil.height, prediction_pil.height)
    ...
    new_img.paste(image_pil, (0, 0))
    # new_img.paste(label_pil, (image_pil.width, 0))
    new_img.paste(prediction_pil, (image_pil.width, 0))
...
def test_single_image(image_path, net, output_dir, FLAGS):
    ...
    visualize_and_save(resized_image, prediction, output_dir, file_name)
...
def Inference(FLAGS):
    ...
    for image_file in tqdm(image_files):
        ...
        test_single_image(image_path, net, output_dir, FLAGS)
    ...
    #     if key < 2:
    #         if len(value)>0:
    #             dice,hd95, iou = zip(*[(value_item[0], value_item[1],value_item[2]) for value_item in value])
    #             non_zero_dice = [value for value in dice if value>0.1]
    #             non_zero_hd95 = [value for value in hd95 if value>0.1]
    #             non_zero_iou= [value for value in iou if value > 0.1]
    ...
    #                   f"IOU: mean: {round(np.mean(non_zero_iou), 2)}, STD: {round(np.std(non_zero_iou), 2)},")
