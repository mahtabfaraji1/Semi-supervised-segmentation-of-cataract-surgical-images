...
def calculate_focal_loss_alpha(directory_path):
    ...
    alpha = {cls: total_samples / count for cls, count in class_counts.items()}
    sum_weights = sum(alpha.values())
    alpha_normalized = {cls: weight / sum_weights for cls, weight in alpha.items()}
    return list(alpha_normalized.values())

def calculate_weights_ce(directory_path):
    class_counts = count_classes_in_masks(directory_path)
    total_samples = sum(class_counts.values())
    n_classes = sum(class_counts)
    alpha = {cls: total_samples / (n_classes*count) for cls, count in class_counts.items()}
    sum_weights = sum(alpha.values())
    alpha_normalized = {cls: weight / sum_weights for cls, weight in alpha.items()}
