...
labels = [
    ...
    Label(name, idx + 4, (idx + 4, idx + 4, idx + 4), idx + 4)
    for idx, name in enumerate(instrument_names)
]
case_list = os.listdir(data_folder)
instrument_label_map = {label.name: label.id for label in labels if label.name in instrument_names}

for p in range(len(case_list)):
    if case_list[p] != '.DS_Store':
        case_folder = data_folder + '/' + case_list[p]
        print("case_number",case_list[p])
            ...
            if jlist[i] != '.DS_Store':
                ...
                if jlist[i] != "SYNAPSE_METADATA_MANIFEST.tsv":
                    with open(file_name, "r") as read_file:
                        data = json.load(read_file)
                    # with open(case_json_folder + '/' + jlist[i], "r",encoding='utf-8') as read_file:
                    #     data = json.load(read_file)
                ...
                for j in range(len(objects)):
                    ...
                    if title in instrument_names:
                        print(title)
                        draw1.polygon(ext, fill=instrument_label_map[title])
