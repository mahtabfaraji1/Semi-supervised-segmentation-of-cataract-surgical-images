...
main_folder = "/Users/mahtab/Downloads/SSL4MIS/segmentation_project/data/100_labeled_data_semisupervised/test/"
...
for video_folder in os.listdir(main_folder):
    ...
    for mask_filename in os.listdir(labels_folder):
        if mask_filename.endswith(".png") or mask_filename.endswith(".jpg"):
            ...
            # Process the mask to keep only relevant classes
            print(f"image_name: {mask_filename} _ labels_two_class: {np.unique(mask)}")
            filtered_mask = process_mask(mask)
...
print("Processing complete! All filtered masks are saved in their respective Labels_2_class folders.")
